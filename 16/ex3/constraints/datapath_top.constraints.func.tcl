...
set clock_periods(clk_i) ${clk_125_period}
...
# Input ports
set_input_delay  $tight_core_input_setup -max -clock ${clock_name}_virtual [get_ports * -filter "direction==in && full_name != clk_i && full_name != rst_ni"]
set_input_delay  $relaxed_core_input_hold  -min -clock ${clock_name}_virtual [get_ports * -filter "direction==in && full_name != clk_i && full_name != rst_ni"] -add
# Output ports
set_output_delay $relaxed_core_output_setup -max -clock ${clock_name}_virtual [get_ports * -filter "direction==out"]
set_output_delay $relaxed_core_output_hold  -min -clock ${clock_name}_virtual [get_ports * -filter "direction==out"] -add
