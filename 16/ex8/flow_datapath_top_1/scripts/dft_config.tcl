...
# Time-stamp: <2025-10-25 17:33:35 qftele>
###############################################
    ...
    if {[llength [get_db flow_vars_scan_abstracts]]} {
        foreach scan_abs [get_db flow_vars_scan_abstracts] {
            ...
            foreach scan_inst [get_db insts -if {.base_cell.base_name==${inst_name}}] {
                    connect -remove_multi_driver \
                        -prefix scan_mode_ \
                        [get_db ports scan_mode_[get_db flow_vars_design_name]] \
                        [get_db $scan_inst .pins -if {.base_name == scan_mode*}]
                    connect -remove_multi_driver \
                        -prefix scan_enable_ \
                        [get_db ports scan_enable_[get_db flow_vars_design_name]] \
                        [get_db $scan_inst .pins -if {.base_name == scan_enable*}]
